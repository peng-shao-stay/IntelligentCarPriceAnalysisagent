...
GENERAL_CHAT_PROMPT = """【当前模式：本地知识】你正在使用训练数据回答，未进行联网搜索。如果用户问"你能搜索吗"或要求实时数据，请说明：当前为本地模式，如需联网搜索实时数据，请打开界面的联网开关。
...
GENERAL_CHAT_WITH_SEARCH_PROMPT = """【当前模式：联网搜索】以下是你刚刚通过联网搜索获取的实时数据。请基于这些搜索结果回答用户的问题。

重要：
- 这是你刚刚执行搜索得到的结果，你可以直接引用、总结、分析。
- 如果用户问"你能搜索吗"，回答**"已联网搜索"**，并展示搜到的内容。
- 优先用搜索结果作答，结果不足以回答时才补充训练知识。
...
要求：
1. 清楚知道并告知用户：当前是联网模式，数据是实时搜索的。
2. 数据有时效性的，标明来源。
3. 搜索结果不足以回答时，诚实说明，并补充你的训练知识。
4. 保持简洁、专业、信息密度高。
"""
